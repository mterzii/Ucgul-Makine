...
import BOTASLOGO from '../images/references/Botaslogo.png';
...
const references = [
  ...
  { name: 'BOTAS', industry: 'Enerji', image: BOTASLOGO },
];

type RefItem = { name: string; industry: string; image?: string };

const ReferenceCard = ({ refItem }: { refItem: RefItem }) => {
  return (
    <div className="group relative flex-shrink-0 w-80 md:w-96 mx-4 p-7 bg-white border-2 border-gray-200 rounded-2xl hover:border-orange-500 hover:shadow-2xl hover:shadow-orange-500/20 transition-all duration-300 hover:-translate-y-2 overflow-hidden cursor-pointer">
      ...
      <div className="relative">
        ...
        <div className="h-24 md:h-28 bg-gradient-to-br from-gray-100 to-gray-50 rounded-xl flex items-center justify-center mb-6 group-hover:from-orange-100 group-hover:to-orange-50 transition-all duration-300 group-hover:scale-[1.02] transform overflow-hidden px-4">
          {refItem.image ? (
            <img
              src={refItem.image}
              alt={`${refItem.name} logo`}
              className="max-h-20 max-w-[240px] object-contain"
              loading="lazy"
              onError={(e) => {
                // logo kırılırsa kart boş kalmasın
                e.currentTarget.style.display = 'none';
              }}
            />
          ) : null}

          {/* img gizlenirse fallback görünür */}
          {!refItem.image ? (
            ...
            <div className="hidden group-[.img-hidden]:flex w-12 h-12 bg-gradient-to-br from-orange-400 to-orange-500 rounded-lg items-center justify-center text-white font-bold text-lg">
              {refItem.name?.[0] ?? 'R'}
            </div>
          )}
        </div>
    ...
    <div className="pt-32 pb-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={marqueeReveal.ref} style={marqueeReveal.style} className="mb-20 overflow-hidden">
          ...
          {/* track genişliği içeriğe göre büyüyor -> -50% doğru çalışıyor */}
          <div className="marquee-track">
            {references.map((ref, i) => (
              <ReferenceCard key={`a-${i}`} refItem={ref} />
            ))}
            {references.map((ref, i) => (
              <ReferenceCard key={`b-${i}`} refItem={ref} />
            ))}
          </div>
        </div>
        ...
        <div ref={statsReveal.ref} style={statsReveal.style} className="bg-gray-50 rounded-lg p-12 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="group relative p-6 bg-white rounded-xl border-2 border-gray-200 hover:border-orange-500 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:shadow-orange-500/20 overflow-hidden cursor-pointer">
              <div className="absolute inset-0 bg-gradient-to-br from-orange-500/0 to-orange-500/0 group-hover:from-orange-500/5 group-hover:to-orange-500/10 transition-all duration-300" />
              <div className="relative">
                <div className="text-3xl font-bold text-orange-500 mb-2 group-hover:scale-110 transform transition-transform duration-300">
                  200+
                </div>
